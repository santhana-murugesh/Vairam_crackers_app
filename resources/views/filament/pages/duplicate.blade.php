<x-filament::page>
    ...
    @php
        $products = App\Models\BillingProduct::all();
    ...
    <select name="product_name[]" class="product-select-1 form-select rounded-md w-full md:w-1/2 h-full product-1 my-2" id="product-select-1" onchange="updatePrice(1)">
        <option selected disabled>Select a Product</option> <!-- Changed to prompt -->
...
<div id="price-display-1" class="mt-2">Price: ₹0</div>

            <!-- Quantity and Price Input -->
            <div class="productinputone flex gap-2 justify-between h-10 body">
                <input type="number" class="rounded-md w-full lg:w-32" id="quantity-1" name=""
                    min="1" placeholder="Quantity" oninput="updatePrice(1)">
            ...
            <div class="flex" id="second-div" style="display: none;">
                <div class="w-1/2">
                    ...
                    <select tabindex="0" name="product_name[]" class="form-select rounded-md md:w-1/2 h-full product-${newRowId}"
            ...
            <h1 class="text-2xl font-semibold mb-5 text-center" style=" color: #123E84">Customer Details</h1>
...
<script>
  function updatePrice(rowId) {
    ...
    var productSelect = document.getElementById('product-select-' + rowId);
    ...
    // Set total price
    document.getElementById('total-' + rowId).value = totalPrice;
  }

  // Delete row function (for removing a product row)
  function deleteRow(button) {
...
<script>
function updatePrice(rowId) {
    const selectElement = document.getElementById(`product-select-${rowId}`);
    ...
    const quantity = quantityInput.value || 0; // Default to 0 if empty
    const total = price * quantity;

    totalInput.value = total ? `₹${total}` : ''; // Format the total as currency
}
</script>
<script>
    function updatePrice(selectId) {
        const selectElement = document.getElementById(`product-select-${selectId}`);
        const selectedOption = selectElement.options[selectElement.selectedIndex];
        const price = selectedOption.getAttribute('data-price');
        
        // Update the price display
        document.getElementById(`price-display-${selectId}`).innerText = `Price: ₹${price}`;
    }
</script>
</x-filament::page>
